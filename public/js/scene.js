...
class Scene {
  ...
  checkRaycaster(){
    ...
    for (let i = 0; i < this.previousInteractions.length; i++){
      let obj = this.previousInteractions[i];
      obj.scale.set(1,1,1);
      // obj.material.color.set.userData.originalColor(); // not sure how to set back to original color
    }
    //check raycaster
  ...
  onMouseUp(){
   if (this.activeSphereIntersection){
